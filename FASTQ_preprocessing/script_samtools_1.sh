...
# Archivos de entrada
FILTER_SAM="../Results/solap_A12_reg_L001_ALIGNED_filtered.sam"
INPUT_SAM="../Results/solap_A12_reg_L001.sam"
...
samtools view "$FILTER_SAM" | awk '{print $1}' > discard_reads_1.txt

# Paso 2: Filtrar las lecturas de INPUT_SAM que no están en discard_reads.txt
samtools view -h "$INPUT_SAM" | grep -vFf discard_reads_1.txt | samtools view -Sb - > filtered_output_1.bam

# Paso 3: Convertir el BAM filtrado a FASTQ
samtools fastq filtered_output_1.bam -1 "$OUTPUT_FASTQ" > "$OUTPUT_FASTQ"

# Limpieza (opcional)
rm discard_reads_1.txt
